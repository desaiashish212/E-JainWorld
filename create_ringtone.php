		...
		if(!file_exists($date))
		{
			 ...
			 foreach($_FILES['files']['tmp_name'] as $key => $tmp_name )
			 {
				...
				$file_type=$_FILES['files']['type'][$key];
					...
					if($count === 0)
					{
						$stmt = $conn->prepare("insert into ringtone (mtitle,url) values (?,?)");
						...
						$count++; 	// on next iteration execute else
					   	...
					   	$_SESSION["MSG"]="Ringtone not added, Try Again";
				...
				//	$htitle=(isset($_POST['htitle']) ? $_POST['htitle'] : null);
					...
					if($count === 0)
					{
						...
						$count++; 	// on next iteration execute else
					   	...
					   	$_SESSION["MSG"]="Ringtone added successfully.";
						header("Location: ringtone.php");
